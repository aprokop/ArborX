              ...
              bool binary = false)
{
  ...
  using Geometry = std::decay_t<decltype(
      Access::get(std::declval<Primitives>(), std::declval<size_t>()))>;
  ...
  Kokkos::View<Geometry *, MemorySpace> primitives_view(
      Kokkos::ViewAllocateWithoutInitializing("primitives"), n);
  Kokkos::parallel_for(
...
auto loadData(std::string const &filename, bool binary)
{
  ...
  for (int i = 0; i < n; ++i)
    IOTraits<Geometry>::read(input, points(i), binary);
  input.close();
