...
{
  Kokkos::Profiling::pushRegion("ArborX::HDBSCAN::dendrogram::sort_edges");
  ...
  Kokkos::View<float *, MemorySpace> negative_weights(
      Kokkos::view_alloc(Kokkos::WithoutInitializing,
                         "ArborX::HDBSCAN::mst_distances"),
      edges.extent(0));
  Kokkos::parallel_for(
      "ArborX::HDBSCAN::compute_edges_distances",
...
dendrogramBFS(ExecutionSpace const &exec_space, MST sorted_mst_edges)
{
  Kokkos::Profiling::pushRegion("ArborX::HDBSCAN::dendrogram_bfs");
  ...
  Kokkos::Profiling::pushRegion(
      "ArborX::HDBSCAN::dendrogram_bfs::copy_to_host");
  ...
  auto incident_offsets_host = Kokkos::create_mirror_view_and_copy(
      Kokkos::HostSpace{}, incident_offsets);
  auto incident_edges_host =
      Kokkos::create_mirror_view_and_copy(Kokkos::HostSpace{}, incident_edges);

  Kokkos::Profiling::popRegion();
  Kokkos::Profiling::pushRegion("ArborX::HDBSCAN::dendrogram_bfs::bfs");

  std::vector<int> cached_vertex_edge(n, n);
  std::function<int(int, int const, int const)> visit;
  visit = [&sorted_mst_edges_host, &incident_offsets_host, &incident_edges_host,
           &cached_vertex_edge,
           &visit](int const vertex_index, int const edge_index,
                   int const edge_from) {
    if (cached_vertex_edge[vertex_index] < edge_index)
    {
      // This vertex has been processed before for an edge with a larger than
      // edge_index index. As the result of the processing is smaller than
      // edge_index, it means that it is going to be the same answer for
      // edge_index, and we may reuse the cached.
      return cached_vertex_edge[vertex_index];
    }

    int first_smaller_edge_index = -1;
    for (int k = incident_offsets_host(vertex_index);
         k < incident_offsets_host(vertex_index + 1); ++k)
    {
      int incident_edge_index = incident_edges_host(k);

      if (incident_edge_index == edge_from)
      {
        // Skip the edge we came from to this vertex so that we don't loop.
        continue;
      }

      if (incident_edge_index < edge_index)
      {
        // Don't need to go further in the direction of edges with smaller
        // index, they are terminal for this edge_index. This can conceptually
        // be understood from the following: if we processed the dendrogram
        // from top down, removing largest edges one-by-one, it would split
        // MST in subgraphs.
        if (incident_edge_index > first_smaller_edge_index)
          first_smaller_edge_index = incident_edge_index;
        continue;
      }

      int incident_vertex_index =
          (sorted_mst_edges_host(incident_edge_index).source == vertex_index
               ? sorted_mst_edges_host(incident_edge_index).source
               : sorted_mst_edges_host(incident_edge_index).target);

      first_smaller_edge_index = std::max(
          first_smaller_edge_index,
          visit(incident_vertex_index, edge_index, incident_edge_index));
    }
    cached_vertex_edge[vertex_index] = first_smaller_edge_index;

    return first_smaller_edge_index;
  };

  Kokkos::View<int *, MemorySpace> edge_parents(
      Kokkos::view_alloc(Kokkos::WithoutInitializing,
                         "ArborX::HDBSCAN::edge_parents"),
      n - 1);
  auto edge_parents_host = Kokkos::create_mirror_view(edge_parents);
  // NOTE: this MUST be done in decreasing order. Otherwise, caching does not
  // work properly and may produce wrong results.
  for (int edge_index = n - 2; edge_index > 0; --edge_index)
  {
    edge_parents_host(edge_index) = std::max(
        visit(sorted_mst_edges_host(edge_index).source, edge_index, edge_index),
        visit(sorted_mst_edges_host(edge_index).target, edge_index,
              edge_index));
  }

  Kokkos::Profiling::popRegion();
  Kokkos::Profiling::pushRegion("ArborX::HDBSCAN::dendrogram::copy_to_device");
...
template <typename ExecutionSpace, typename MST>
Kokkos::View<int *, typename MST::memory_space>
dendrogramBottomUp(ExecutionSpace const &exec_space, MST sorted_mst_edges)
{
  Kokkos::Profiling::pushRegion("ArborX::HDBSCAN::dendrogram_bfs");
  ...
  Kokkos::Profiling::pushRegion("ArborX::HDBSCAN::dendrogram::copy_to_host");
  ...
  Kokkos::Profiling::popRegion();
  Kokkos::Profiling::pushRegion("ArborX::HDBSCAN::dendrogram::bottom_up");
  ...
  int const num_vertices = n;
  int const num_edges = n - 1;
  std::vector<std::vector<int>> edge_cuts(num_edges + num_vertices);
  int const untouched = -1;
  std::vector<int> flags(n, untouched);
  for (int k = n - 1; k < 2 * n - 1; ++k)
  {
    ...
    edge_cuts[i].insert(
        edge_cuts[i].end(),
        &incident_edges_host(incident_offsets_host(i - num_edges)),
        &incident_edges_host(incident_offsets_host(i + 1 - num_edges)));

    do
    {
      auto &edge_cut = edge_cuts[i];

      auto parent_it = std::max_element(edge_cut.begin(), edge_cut.end());
      int parent = *parent_it;

      // erase last element
      *parent_it = edge_cut.back();
      edge_cut.pop_back();

      edge_parents_host(i) = parent;

      int other_child =
          Kokkos::atomic_compare_exchange(&flags[parent], untouched, i);
      if (other_child == untouched)
        break;

      edge_cuts[parent].reserve(edge_cuts[i].size() +
                                edge_cuts[other_child].size());
      for (int child : {i, other_child})
        edge_cuts[parent].insert(edge_cuts[parent].end(),
                                 edge_cuts[child].begin(),
                                 edge_cuts[child].end());
...
{
  Kokkos::Profiling::pushRegion("ArborX::HDBSCAN::dendrogram_union_find");
  ...
  Kokkos::View<int *, MemorySpace> representative_edges(
      Kokkos::view_alloc(Kokkos::WithoutInitializing,
                         "ArborX::HDBSCAN::representative_edges"),
      n);
  ...
  Kokkos::Profiling::popRegion();
  Kokkos::Profiling::pushRegion("ArborX::HDBSCAN::dendrogram::union_find");
...
{
  Kokkos::Profiling::pushRegion("ArborX::HDBSCAN::compute_flat_clustering");
  ...
  Kokkos::parallel_for(
      "ArborX::HDBSCAN::compute_num_descendants",
...
findAlphaEdges(ExecutionSpace const &exec_space, MST const &sorted_mst_edges)
{
  ...
  Kokkos::View<int *, MemorySpace> alpha_edge_indices(
      Kokkos::view_alloc(Kokkos::WithoutInitializing,
                         "ArborX::HDBSCAN::alpha_edge_indices"),
      num_edges);
  int num_alpha_edges;
...
{
  Kokkos::Profiling::pushRegion("ArborX::HDBSCAN::build_alpha_mst");

  int const num_alpha_edges = alpha_edge_indices.extent(0);

  EulerTour alpha_euler_tour(
      Kokkos::view_alloc(Kokkos::WithoutInitializing,
                         "ArborX::HDBSCAN::alpha_euler_tour"),
      2 * num_alpha_edges);
  Kokkos::parallel_for(
      "ArborX::HDBSCAN::pick_alpha_euler_tour",
  ...
  Kokkos::parallel_for(
      "ArborX::HDBSCAN::compute_inv_permute",
  ...
  MST alpha_mst_edges(Kokkos::view_alloc(Kokkos::WithoutInitializing,
                                         "ArborX::HDBSCAN::alpha_mst_edges"),
                      num_alpha_edges);
  Kokkos::parallel_for(
      "ArborX::HDBSCAN::compute_alpha_mst",
      Kokkos::RangePolicy<ExecutionSpace>(exec_space, 0, num_alpha_edges),
      KOKKOS_LAMBDA(int i) {
