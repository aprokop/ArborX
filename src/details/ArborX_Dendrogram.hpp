...
struct Dendrogram
{
  ...
  {
    Kokkos::Profiling::ScopedRegion guard("ArborX::Dendrogram::Dendrogram");
      ...
      Kokkos::View<int *, MemorySpace> &parents)
  {
    ...
    Kokkos::View<int *, MemorySpace> global_map(
        ...
        num_global_edges);
    Details::KokkosExt::iota(exec_space, global_map);

    int level = 0;
    do
    {
      Kokkos::Profiling::ScopedRegion level_guard("ArborX::Dendrogram::level_" +
                                                  std::to_string(level));
      ...
      auto const largest_alpha_index =
          Details::KokkosExt::lastElement(exec_space, alpha_edge_indices);
