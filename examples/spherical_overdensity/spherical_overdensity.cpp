...
int main(int argc, char *argv[])
{
  ...
  if (validate)
  {
    ...
    for (int i = 0; i < num_halos; ++i)
    {
      ...
      if (!matched)
      {
        printf("counts for halo tag %" PRId64 " do not match: validation = [",
               in_fof_halo_tags(i));
        for (int bin_id = 1; bin_id < num_sod_bins; ++bin_id)
    ...
    for (int i = 0; i < num_halos; ++i)
    {
      ...
      if (!matched)
      {
        printf("masses for halo tag %" PRId64
               " do not match: relative errors [",
               in_fof_halo_tags(i));
        for (int bin_id = 1; bin_id < num_sod_bins; ++bin_id)
        {
          auto error =
    ...
    for (int i = 0; i < num_halos; ++i)
    {
      ...
      if (a != b)
      {
        ...
        max_error = std::max(error, max_error);
        printf("%d rdelta for halo tag %" PRId64 " do not match: "
